        ...
        <div className="flex items-center justify-center gap-1.5 mt-10 font-sans">
            ...
            {pages.map((page, i) =>
                page === "..." ? (
                    <span
                        key={`dots-${i}`}
                        className="w-8 h-8 flex items-center justify-center text-foreground/30 text-sm select-none"
                    >
                        ···
                    </span>
                ) : (
...
export default function PropiedadesContent({
    ...
    const [opType, setOpType] = useState(
        searchParams.get("operacion") ||
            initialOptions["tipo_operacion"]?.[0] ||
            ""
    );
        ...
        let filtered = initialProperties.filter((p) => {
            if (searchQuery) {
                ...
                const searchableText =
                    `${p.title || ""} ${p.description || ""} ${p.localidad || ""} ${p.location || ""} ${p.property_type || ""} ${p.operation_type || ""} ${p.features ? (Array.isArray(p.features) ? p.features.join(" ") : p.features) : ""}`.toLowerCase();
            ...
            if (
                ...
                !p.operation_type
                    ?.toLowerCase()
                    .includes(opType.toLowerCase().trim())
            )
                return false;
            ...
            <aside className="w-full lg:w-[320px] shrink-0">
                <div className="bg-card rounded-xl shadow-sm border border-border-card p-6 sticky top-24 font-sans transition-colors duration-300">
                    ...
                    <div className="space-y-8">
                        <div>
                            <h4 className="font-bold text-sm text-foreground/80 mb-3 uppercase tracking-wider">
                                Buscar
                            </h4>
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {initialOptions.tipo_operacion?.map(
                                    (op, idx) => (
                                        <button
                                            key={`op-${idx}`}
                        ...
                        <div>
                            ...
                            <div className="relative">
                                ...
                                >
                                    ...
                                    {initialOptions.tipo_propiedad?.map(
                                        (t, idx) => (
                                            <option
                                                key={`prop-${idx}`}
                                                value={t}
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-4 gap-2">
                                {["1", "2", "3", "4+"].map((amb, idx) => {
                                    const isSelected =
                                        ambientesSelected.includes(amb);
                                    return (
                                        <button
                                            key={`amb-${idx}`}
                                            onClick={() =>
                                                handleAmbienteToggle(amb)
                                            }
                                            className={`py-2 px-1 text-center font-bold text-sm rounded-lg border transition-all duration-200 ${isSelected ? "bg-primary/10 border-primary text-primary" : "bg-card border-border-card text-foreground/60 hover:bg-input"}`}
                                        >
                                            {amb}
                                        </button>
                                    );
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-2">
                                <div className="relative flex-1">
                                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-foreground/40 font-medium">
                                        $
                                    </span>
                                    <input
                                        type="number"
            ...
            <div className="flex-1 flex flex-col min-h-[600px] font-sans">
                <div className="mb-6 flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4 border-b border-border-card pb-4">
                    ...
                    <div className="flex items-center gap-3">
                        <span className="text-sm font-semibold text-foreground/50 uppercase tracking-wider hidden sm:inline">
                            Ordenar por:
                        </span>
                        <select
                ...
                {filteredProperties.length === 0 ? (
                    <div className="py-20 flex flex-col items-center justify-center text-center bg-card rounded-xl border border-border-card shadow-sm px-4 flex-1">
                        ...
                        <p className="text-foreground/50">
                            Probá ajustando los filtros de la izquierda.
                        </p>
                    </div>
                ) : (
                    <>
